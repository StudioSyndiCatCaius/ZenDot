...
scons -C "%GODOT_SOURCE%" platform=windows target=template_release arch=x86_64 -j%SCONS_JOBS% > "%TEMP%\zd_scons.tmp" 2>&1
...
scons -C "%GODOT_SOURCE%" platform=windows target=template_debug arch=x86_64 -j%SCONS_JOBS% > "%TEMP%\zd_scons.tmp" 2>&1
set BUILD_ERR=!ERRORLEVEL!
type "%TEMP%\zd_scons.tmp"
type "%TEMP%\zd_scons.tmp" >> "%LOG_FILE%"
del "%TEMP%\zd_scons.tmp" 2>nul
...
set ZIP_FILE=%ROOT_DIR%\windows_templates.zip
set TPZ_FILE=%ROOT_DIR%\windows_templates.tpz
...
rename "%ZIP_FILE%" "windows_templates.tpz"
