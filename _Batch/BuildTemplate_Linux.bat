...
set ROOT_DIR=%CD%
popd
set GODOT_SOURCE=%ROOT_DIR%
set OUTPUT_DIR=%ROOT_DIR%\export_templates
set VERSION_STRING=4.6.stable.custom_build
set SCONS_JOBS=8
:: ============================================================
:: NOTE: Cross-compiling Linux templates from Windows requires
:: a MinGW-w64 or llvm-mingw toolchain targeting linux-gnu.
:: Set the appropriate compiler in your SCons environment or
:: pass use_mingw=yes / use_llvm=yes as needed.
:: ============================================================

set LOG_FILE=%OUTPUT_DIR%\build_log_linux.txt

echo.
echo ============================================================
echo  Godot Linux Export Template Builder (cross-compile)
echo ============================================================
echo.
...
scons -C "%GODOT_SOURCE%" platform=linuxbsd target=template_release arch=x86_64 -j%SCONS_JOBS% > "%TEMP%\zd_scons.tmp" 2>&1
set BUILD_ERR=!ERRORLEVEL!
type "%TEMP%\zd_scons.tmp"
type "%TEMP%\zd_scons.tmp" >> "%LOG_FILE%"
del "%TEMP%\zd_scons.tmp" 2>nul
...
scons -C "%GODOT_SOURCE%" platform=linuxbsd target=template_debug arch=x86_64 -j%SCONS_JOBS% > "%TEMP%\zd_scons.tmp" 2>&1
set BUILD_ERR=!ERRORLEVEL!
type "%TEMP%\zd_scons.tmp"
type "%TEMP%\zd_scons.tmp" >> "%LOG_FILE%"
del "%TEMP%\zd_scons.tmp" 2>nul
...
set RELEASE_SRC=%GODOT_SOURCE%\bin\godot.linuxbsd.template_release.x86_64
set DEBUG_SRC=%GODOT_SOURCE%\bin\godot.linuxbsd.template_debug.x86_64
set RELEASE_DST=%OUTPUT_DIR%\linux_release.x86_64
set DEBUG_DST=%OUTPUT_DIR%\linux_debug.x86_64
...
set ZIP_FILE=%ROOT_DIR%\linux_templates.zip
set TPZ_FILE=%ROOT_DIR%\linux_templates.tpz

if exist "%ZIP_FILE%" del "%ZIP_FILE%"
if exist "%TPZ_FILE%" del "%TPZ_FILE%"

powershell -Command "Compress-Archive -Force -Path '%OUTPUT_DIR%\linux_release.x86_64','%OUTPUT_DIR%\linux_debug.x86_64','%OUTPUT_DIR%\version.txt' -DestinationPath '%ZIP_FILE%'"
...
rename "%ZIP_FILE%" "linux_templates.tpz"
